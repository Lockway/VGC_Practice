...
`song.csv`의 곡 중 중복 없이 3곡을 무작위로 뽑고, `rival.csv`의 라이벌 점수를 함께 보여주는 모바일 웹 페이지입니다. 설치나 서버 프로그램 없이 GitHub Pages에서 동작합니다.
...
곡을 추가하거나 삭제하려면 `song.csv`의 `곡명` 열을 편집하면 됩니다. 점수는 `rival.csv`의 `곡명`, `첫번째 점수`, `두번째 점수` 열에서 읽습니다. 두 번째 점수가 `0`이면 첫 번째 점수만 표시하고, `rival.csv`에 없는 곡은 `0`으로 표시합니다. 웹 페이지가 열릴 때 두 CSV 파일을 읽습니다.
